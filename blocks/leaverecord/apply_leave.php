...
if ($data = $mform->get_data()){
    ...
    if($leave_status==0){ // full day leave
        $ret = apply_leave($full_day,$leave_reason,$leave_status);
    }
    if($leave_status==3){ // 3 = MD
        $ret = md_leave($leave_from,$leave_to,$leave_reason);
    }
    //var_dump(http_response_code());
    if (gettype($ret) == 'string'){
        echo html_writer::div('Error in updating records :'.$ret);
    }else {
        if (property_exists($ret, 'detail')){
            echo html_writer::div('Error in applying for leave : '.$ret->detail);    
        } else{
            echo html_writer::div('Leave has been successfully added');
        }
    }
    echo $OUTPUT->continue_button($CFG->wwwroot.'/blocks/leaverecord/apply_leaves.php?blockid='.$_GET['blockid']);
} else {
    $mform->display();
}
echo $OUTPUT->footer();
