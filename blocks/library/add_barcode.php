<?php
/*
    This page provide functionality to Add Barcodes to Books in Library//
*/
...
$PAGE->requires->js('/blocks/library/js/jquery.scannerdetection.min.js'); // js require for barcode scanner machine //
$PAGE->requires->js('/blocks/library/js/lib_barcode_add.js'); // js require for barcode scanner machine //
echo $OUTPUT->header();
...
if (is_branch_admin() || is_secondary_user()){
    ...
    if ($data = $mform->get_data()){
        ...
        $addBook_record->id      =    $data->book; // data came from form input
        $addBook_record->barcode =    $data->book_barcode; // data came from form input
        $addBook_record->is_scanned = 1; // initially status is 1
        $DB->update_record('lib_bookmaster',$addBook_record); // update book record on perticular id
