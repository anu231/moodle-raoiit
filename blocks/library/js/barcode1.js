...
// barcode //
var field_selector1 = '#id_barcode';
$(field_selector1).focus();
$(field_selector1).scannerDetection(function(barcode,qty){
    console.log(barcode);
    //fetch info for this book
    //via ajax
     // $.getJSON({
        ...
        success: function(resp){
            ...
            price=resp.price;
            var main_region = document.getElementById('region-main');
            var book_barcode = document.getElementById('id_barcode').value;
            var lenghth_barcode = book_barcode.length;
           // alert(lenghth_barcode);
            if(lenghth_barcode==13){
                var para = document.createElement("p");
