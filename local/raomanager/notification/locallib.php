...
function notification_send($notif) {
    ...
    if( !$students ) // No student matched the criteria/filter
        return FALSE;
...
function notification_get_profiles($filter=null) {
    ...
    foreach ($users as $user) {
        ...
        if($filter['targetyears'] != '*') 
            if(!in_array($userinfo->targetyear, $filter['targetyears'])) // Check if user belongs to selected target years
                continue;
        if($filter['centers'] != '*')
            if(!in_array($userinfo->center, $filter['centers'])) // Check if user belongs to selected centers
                continue;
        if($filter['batches'] != '*')
            if(!in_array($userinfo->batch, $filter['batches'])) // Check if usser belongs to selected batches
                continue;
