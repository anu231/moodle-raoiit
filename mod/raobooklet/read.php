...
if ( $page == -1 && $get_items == 0 ) {
    ...
    $path = "$CFG->dataroot/raobooklets/".$instance.'/images/';
    $images = glob($path.'*');
    $json = array();
    foreach ($images as $image) {
        $exp = explode('/', $image);
        $number = explode('.', $exp[7])[0];
        $json[$number] = "$CFG->wwwroot/mod/raobooklet/read.php?instance=$instance&page=".$number;
    }
