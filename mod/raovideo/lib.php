...
function raovideo_add_instance($raovideo, $mform=NULL){
    ...
    if($raovideo->bookletid != 0){
        // Assign an existing booklet(booklet != 0)
        // $raovideo->name = $DB->get_record('raovideo_info', array('bookletid' => $raovideo->bookletid))->name;
        ...
        $raovideo->bookletid = $file->get_id(); // Override bookletid(0) with $fileid. (This is how it's designed)
        $DB->insert_record('raovideo_info', $raovideo); // Save metadata
...
*/
function raovideo_add_or_update_feedback($bookletid, $feedback) {
    global $DB, $USER;
