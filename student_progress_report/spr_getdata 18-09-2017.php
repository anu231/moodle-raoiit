...
function get_subjectwise_ques($paper_data){
  ...
  if($paper_data['stream']== 'Eng' && strpos($paper_type, 'main') !== false){
  ...
  if($paper_data['stream']== 'Med' && strpos($paper_type, 'aiims') !== false){
    $subjectwise_ques += array("phy_ques" => 60);
    $subjectwise_ques += array("chem_ques" => 60);
    $subjectwise_ques += array("bio_ques" => 80);
    $subjectwise_ques += array("phy_total_marks" => 60);
    $subjectwise_ques += array("chem_total_marks" => 60);
    $subjectwise_ques += array("bio_total_marks" => 80);
  }
...
function user_test_report($paper_data){
  ...
  $neg_marks_per_wrong=$paper_data['scnegmarks'];
    ...
    for($i=0;$i<$elementCount;$i++){
        ...
        if($spr_data[$i]['pcorr']!=0 && $spr_data[$i]['pwrong']!=0){
        ...
        if($spr_data[$i]['ccorr']!=0 && $spr_data[$i]['cwrong']!=0){
          ...
          $chem_corr_percent=0;
          $chem_wrong_percent=0;
          $chem_unattempt_percent=0;
          $chem_corr_accuracy=0;
        }
        ...
        if($spr_data[$i]['mcorr']!=0 && $spr_data[$i]['mwrong']!=0){
          ...
          $maths_corr_percent=0;
          $maths_wrong_percent=0;
          $maths_unattempt_percent=0;
          $maths_corr_accuracy=0;
        }
        ...
        if($spr_data[$i]['bcorr']!=0 && $spr_data[$i]['bwrong']!=0){
